...
class SpottingsController {
  ...
  async getAll(c: ContextType): Promise<SpottingsArrayID> {
    try {
      PotLogger("[SPOTTINGS - ALL] -", `Grabbing from DB...`);
      const pool = new Pool({ connectionString: c.env.DATABASE_URL });
      const adapter = new PrismaPg(pool);
      const prisma = new PrismaClient({ adapter });

      const data = await prisma.spottings.findMany({
        select: { message_id: true, date: true },
        where: {
          date: {
            lte: new Date(), // to avoid false positives
          },
        },
        orderBy: [{ date: "desc" }, { id: "asc" }],
      });

      const renamedData = data.map((data) => ({
        id: data.message_id,
        date: data.date.toISOString(),
      }));
